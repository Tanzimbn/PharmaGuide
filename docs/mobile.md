...
| **M3 — Personal docs UI** | ✅ done | Real two-tab app (replaces the M1/M2 dev harness): a **Library** tab to add / categorize (free-text + suggestions) / replace / delete your PDFs with live ingest status, and an **Ask** tab (scoped query → grounded, page-cited answer + "not covered" badge). *Presentation only — reuses `pipeline`/`lifecycle`/`answer` unchanged. New `src/ui/` design kit (theme + components), `src/screens/`, `useDocuments` hook; **custom JS bottom tab bar** (no nav library → zero new native modules, installed dev build still works). Parity unchanged.* |
| **M4 — LLM settings & access** | ✅ done | BYO-key entry + **device secure storage** (`expo-secure-store` → Keychain/Keystore) for the API key, plus editable LLM base URL / model and the **extract-service URL** — all runtime settings, no more `.env`/compile-time consts. New 3rd **Settings** tab + `src/settings.ts` (load/get/save/clear over secure store; `config.ts` keeps only `DEFAULT_*` first-run seeds). `llm.ts`/`extractClient.ts` read settings at call time; missing key/URL → user-facing "set it in Settings". *Pure prompt builders split into `src/prompt.ts` so parity stays Node-loadable — `query_parity`/`chunk_parity` unchanged. Adds one native module → one-time `expo prebuild` + dev-client reinstall. Extraction stays a **per-user BYO endpoint**, never a shared service (NFR-2).* |
| **M5 — Hardening** | ✅ done (on-device-LLM deferred) | Gold-set eval + **cosine score-guard calibration**. Off-device Python harness (`mobile/tools/eval/run_eval.py`) reuses the backend's real bge-small + chunk logic (int8 parity-established M0) over a PIC/S GMP gold set (n=10, covered + not-covered). Result: covered top-1 cosine 0.71–0.84 vs not-covered 0.44–0.66 — fully separable → **`SCORE_THRESHOLD` 0.3 → 0.68** (gap midpoint, ±0.024 margin for fp32→int8 drift); recall@5 = 0.80. **Latency:** `__DEV__` stage timings (embed / kNN / generate / total) in `retrieve.ts`+`answer.ts`, read from the device console (procedure in app README §Eval). On-device LLM / reranker / extraction + sqlite-vec deferred. |
