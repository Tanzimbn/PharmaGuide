...
| Phase | Deliverable |
| --- | --- |
| **M0 — Inference spike** | RN app loads bge-small as ONNX and embeds a string on-device on both iOS + Android with hardware acceleration. **De-risks the whole plan before further build.** Full checklist + pass/fail gates: [`mobile-m0-spike.md`](mobile-m0-spike.md). |
| **M1 — On-device ingestion** | Pick PDF → extract (per §6) → chunk → embed on-device → store in SQLite+sqlite-vec; status processing→ready→failed. Atomic replace/delete. |
| **M2 — On-device query** | embed query → kNN → rerank → score guard → shared-LLM-proxy call → grounded, page-cited answer + "not covered" fallback. |
